async page => {
 ...
 await page.locator('#feedbackButton').click();await dialog.getByRole('combobox',{name:'처리 상태',exact:true}).selectOption('resolved');
 ...
 const result=await page.evaluate(()=>({count:__v2Mock.tables.v2_feedback.length,status:__v2Mock.tables.v2_feedback[0].status,overflow:document.documentElement.scrollWidth>innerWidth}));
