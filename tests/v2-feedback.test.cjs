...
test('feedback privacy, input validation, retry identity and admin optimistic locking',async()=>{
 ...
 for(const suffix of ['club_member_accounts_and_member_rsvp','v2_independent_schedules','v2_feedback']) {
 ...
 await actor(admin);assert.equal((await list()).rows.length,2);assert.equal((await change()).rows[0].version,2);await deny(()=>change(),'40001');
