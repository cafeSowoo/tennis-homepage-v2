...
assert.deepEqual(result.errors,[]);assert.equal(result.count,0);assert.equal(result.deleted,true);assert.equal(result.safe,true);assert.equal(result.cleared,true);assert.equal(result.overflow,false);console.log(JSON.stringify({viewport,...result},null,2));
