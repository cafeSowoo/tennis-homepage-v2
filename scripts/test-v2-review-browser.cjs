...
assert.match(result.entered.reviewLastSync,/^마지막 확인 · /);
assert.deepEqual(result.photoViewerOpened,{open:true,fullSurfaceClose:true});assert.equal(result.photoViewerClosedByImage,true);assert.equal(result.photoViewerInstantCloseKeepsDetail,true);
