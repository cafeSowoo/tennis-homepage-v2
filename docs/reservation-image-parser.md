...
The V2 homepage includes a reservation-capture sheet/modal that approved active members can open from schedule-add controls without leaving the current page. The `AI Test` navigation item remains visible only to club admins, but it opens the same sheet.
...
- Choosing the capture option opens a bottom sheet on mobile and a centered modal on desktop. No main-view navigation occurs.
...
- Each valid draft can be sent to the existing schedule-creation sheet with `이 일정 가져오기`. The capture sheet hands off to the normal schedule sheet while keeping a single modal-history entry.
