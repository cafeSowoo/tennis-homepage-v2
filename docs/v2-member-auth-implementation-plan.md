...
> 이 문서는 공유 데이터 기반 회원 기능의 초기 설계와 구현 기록이다. 앞으로의 실행 기준은 [V2 독립 운영 전환 계획](v2-independent-operation-transition-plan.md)이다. 아래의 ‘같은 테니스 데이터 공유’ 목표는 전환 전 구조로 보존하며, V2 전용 일정·참석·댓글 구조가 새 권장 방향이다. 카카오 상태 복귀와 댓글 열람 확대의 신규 개발은 보류하고, 로그인·회원 승인·화면·검증 원칙은 재사용한다.
...
- 후속 회원 댓글 작성/본인 삭제 UI와 추가 RLS 정책을 구현했다. 원격 적용·rollback 검증 완료. 기존 V1 owner 정책과 Kakao 댓글 전문 열람 정책은 유지했다.

### 인증 운영 방향 보완 (2026-09-17 합의)

- V1은 기존 Google 관리자 로그인과 운영 기능을 보존한다. Google 사용은 기술적 필수가 아니라 기존 이메일 기반 권한과 운영 환경을 유지하기 위한 선택이며, V1의 카카오 전환은 현재 작업 범위에 포함하지 않는다.
- V2는 카카오 기반 회원 기능을 완성하고, 관리자 기능을 승인 상태·역할 기반으로 단계적으로 통합하는 것을 후속 목표로 한다. 전체 관리자 기능의 역할 기반 전환은 아직 완료되지 않았으며 UI와 DB 권한을 함께 검토한다.
- 김지석 Kakao 계정은 `approved/member`로 유지한다. 같은 회원에 연결된 Google 관리자 계정의 권한을 자동으로 승계하지 않으며 관리자 승격은 별도로 결정한다.
- V1/V2 동시 로그인·로그아웃 격리 검증은 V2 정식 운영 전 최종 점검으로 수행하며, 독립적인 후속 개발을 막는 선행 조건으로 두지 않는다.
- V1 대체·종료는 V2 검증 완료 후 별도로 결정한다. 상세 실행 기준은 [현재 1차 계획](v2-phase-1-plan-2026-09-17.md)을 따른다.
...
8. V1/V2 로그인 session 격리 확인 (V2 정식 운영 전 최종 점검이며 후속 개발의 선행 조건은 아님)
