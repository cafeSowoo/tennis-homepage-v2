...
*   **Place-based titles (Kakao schedules).** Cards and the detail heading show `테니스장 + 코트` (e.g. `달빛공원 12번`, `달빛축제공원 B코트`) instead of the raw Kakao title; tags keep what only the title said (`예약일`, `마감`, `정기모임`). The raw title appears once, small, under the detail heading (`카카오 원문 · …`). The card's second line is `time · surface`, with the time in text color (declined cards stay grey).
*   **Header (member mode).** Approved members see only their avatar in the header; tapping it opens 내 정보. Logout and the schedule-alert toggle live in a 설정 card on 내 정보 (own profile only). The review-mode name button is unchanged.
*   **Images.** Display lightweight WebP copies: `assets/logo-192.webp`, `assets/brand-wordmark.webp`, member photos in `assets/members/opt/` (`*-192.webp` for avatars, 512px for the photo viewer), court photos in `assets/courts/opt/` (the DB keeps original paths; `optimizedCourtImage()` swaps them at render time). Keep originals for the manifest/DB.
